...
2. **团队报告**：跑 `make-report.py` 生成 HTML，按提交者分组列出每个人的修名清单，top 3 戴 medal
3. **VCS 拦截**：把 `pre-commit.bat` 装在 SVN 服务器，新增的违规命名直接被拒绝 commit

规则定义集中在 `src/rules.py`，每个检测器有稳定的 r 编号（r1, r2, ...），文件内置 self-test，改规则跑一下 `python src/rules.py` 就知道有没有破坏现有判定。
...
| r-id | 名字 | 原因 | 自动建议 |
|---|---|---|---|
| r1  | `Slash-Attack-L_v01` | 不允许连字符 | `Slash_Attack_L_V_01` |
| r2  | `Sample__Small_Cymbal` | 双下划线 | `Sample_Small_Cymbal` |
| r3  | `attackDodge` | 缺前缀分隔符 | `XX_AttackDodge` |
| r3  | `BiteIcon` | 缺前缀分隔符 | `XX_BiteIcon` |
| r4  | `MyGame_ArtPush_HeartIcon` | 前缀必须全大写 | （需人工） |
| r5  | `SW_bite` | token 小写开头 | `SW_Bite` |
| r6  | `SM_Test_01_A` | variant 必须在 index 之前 | `SM_Test_A_01` |
| r7  | `BP_Prefab_Block_03B` | variant 黏在 index 后面 | `BP_Prefab_Block_B_03` |
| r8  | `SM_TestA` | variant 黏在名字上 | `SM_Test_A` |
| r9  | `L_WhiteBox_Sub_01PoolRoom` | 数字黏在文本上（中段）| `L_WhiteBox_Sub_01_PoolRoom` |
| r10 | `SM_Footstep1` | 索引黏在文本上（末尾）| `SM_Footstep_01` |
| r11 | `SM_Test_1` | 索引必须补零 | `SM_Test_01` |
| r12 | `MF_DynamicSCurve` | 单字母大写卡在 PascalCase 中间 | （需人工） |
| r13 | `MF_00_FlatNormal` | 前缀后立刻跟数字 token (TBD) | （需人工） |
| r14 | `Box_5C5F67FD` | UE 自动生成的 BSP brush 命名 | （需人工，按用途重命名）|
| r99 | （罕见结构）| unknown structure | （需人工） |

约定：

- **r-id 永久稳定**。reason 文字可改，r 编号不会重新分配。删除的规则其编号保留为占位，不会复用
- **`XX_` 是占位前缀**。r3 的自动建议会加 `XX_`，使用者需要手动替换为正确的类型前缀（`SM_`、`T_`、`BP_` 等）
- **"需人工"**的 case 不能自动改名，因为修复需要语义判断（确定的前缀是什么、卡住的字母代表的真实词是什么、生成 brush 的实际用途等）
- **r13** 是 pending TBD，不是 confirmed violation，由各项目自决是否接受
...
- `output/export.ini` — 本次跑到的所有 asset 名字、ignored 列表和 meta
...
读 validator 的输出，对每个违规文件查 VCS 的最后修改作者，生成 `output/report.html`。每个人有自己的"待修清单"，按违规原因分组，每条都附自动建议。报告里 author 表前三名戴 🥇🥈🥉，第四名以后挂 🫨。
...
**硬编码 skip**（在 `validator.py` 里固定，不受 ignores.ini 控制）：

| 模式 | 原因 |
|---|---|
| `__External*` | UE5 World Partition 自动生成的 external actor 目录 |
| `Content/Splash/` | UE 自动生成的 splash screen 目录 |
| `ProjectThumbnail.uasset` | UE 自动生成的项目缩略图 |

这三个是 UE 通用约定，每个项目都一样，所以不让 ignores.ini 关掉。

## 输出格式

### `verified.ini` / `violation.ini`

按 r-id 分 section。每行一个名字，suggestion 如果存在就作为 value。
...
- `[rN: ...]` section = 已确认违规
- `[TBD: rN: ...]` section = 规则标到了，但项目层面尚未拍板要不要算违规
- 空 value（`name =`）= 没有自动建议，需要人工命名
- 只列名字、不带 value = 这是 TBD section 的格式

### `export.ini`

本次扫描的全量数据，4 个 section：
...
`make-report.py` 读 `[meta]` 拿到所有计数，读 `[paths]` 做 name→path 反查，读 `[ignored_*]` 渲染折叠列表。

### `report.html`

浏览器可渲染的团队报告，结构：

- **标题** `<code>.uasset</code> Name Linter Report` + 生成时间戳（精确到秒）
- **头部统计** `Total Scanned: N assets`、`Ignored: N assets and N directories`
- **折叠 Ignored Directories** 完整路径列表
- **折叠 Ignored Assets** 完整路径列表（scrollable，因为可能很多）
- **Summary** verified / violation / TBD 三个数字 + 百分比
- **Violations by Author** 按作者排序的违规计数表，最左边一列：top 3 戴 🥇🥈🥉，第四名以后 🫨
- **Violations by Reason** 按 r-id 排序的违规分类表
- **Top 20 Dirty Directories** 违规最严重的目录
- **Per-Author Action Lists** 每个作者一个折叠 `<details open>`，按 r-id 分组列出待修文件 + 自动建议
...
1. 打开 `src/rules.py`
2. 在 `REASONS` 字典里追加新 r 编号（**永远 append，不重用旧编号**）
3. 加新的正则常量
4. 在 `classify()` 里加一条分支返回 `(VIOLATION, 'rNN')`
5. （可选）给 `suggest_fix()` 加一个变换，实现 auto rename
6. 在文件底部的 self-test 列表里加 accept / reject case，标注预期 r-id
7. 跑 `python src/rules.py` 验证全部 case 通过
8. 重新跑 `python src/validator.py`

## 已知限制

- **混合大小写缩写**（`AoE`、`IoT`、`MoBA`）会被拒。请用全大写（`AOE`）或纯 PascalCase（`Aoe`）
- **r13（前缀后立刻跟数字 token）目前归类为 TBD**，没有自动建议。这是不是合法 marketplace 风格由各项目自决
...
- **包含单字母 token 的资源包**（input glyph 贴图集合 `T_S_A`、`T_X_LB` 等）需要通过 `ignores.ini` 排除，因为单字母 token 跟 variant slot 在语法上无法区分
- **r3 的自动建议是占位**。`XX_AttackDodge` 这种建议里的 `XX` 不是真前缀，使用者需要手动替换成正确的类型前缀
